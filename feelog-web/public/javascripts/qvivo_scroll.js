...
$.fn.extend({
   diaryUpdate:function() {
            ...
            var updateScroll = function() {
                if (!mouseOver){ // stops scrollbar jumping
                    ...
                    if (ratio < 1) { // content is big enough to show the scrollbar
                        var top = Math.min(Math.round($('.diary').scrollTop() * ratio), $('.diary').height() - scrollBar.height());
                        scrollBar.css({top: top + 'px'});
                        if ((ratio * track.height()) < 50)
                            scrollBar.css({height: '10%'});
                        else
	...
	diaryScroll: function() {
		this.each(function(i) {
			$(this).prepend('<div class="scroll_wrapper"><div class="scroll_track"><div class="scroll_track_footer"><div class="scroll_bar"><div class="scroll_bar_top"></div><div class="scroll_bar_bottom"></div></div></div></div></div>');
			var scrollWrapper = $(this).children('.scroll_wrapper');
			var track = scrollWrapper.children('.scroll_track');
			var content = /*scrollWrapper.children('.content_wrapper');*/$('.diary-content');
			var scrollBar = track.children('div').children('.scroll_bar');
			var mouseOver = false;
			scrollBar.hover(
				function () { mouseOver = true; }, 
			...
			var updateScroll = function() {
				if (!mouseOver){ // stops scrollbar jumping
					var ratio = (track.height() / content.height());
					if (ratio < 1) { // content is big enough to show the scrollbar
						var top = Math.min(Math.round(/*scrollWrapper*/$('.diary').scrollTop() * ratio), $('.diary').height() - scrollBar.height());
						scrollBar.css({top: top + 'px'});
						if ((ratio * track.height()) < 50)
							scrollBar.css({height: '10%'});
						else
            ...
            $(this).scroll(function() { updateScroll(); });
