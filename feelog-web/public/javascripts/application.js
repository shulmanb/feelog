...
function returnFromCreateMood(report_time,mood_val,desc){
    ...
    var mood = {"val":mood_val,"desc":desc,"date":ts.toString()};
    ...
    var p;
    if(norm == 1){
       p = {"name":mood.desc,"y":mood.val,"x":d};
    }else{
       p = {"name":mood.desc,"y":mood.val,"t":d};
    }
    ...
    function renderMoods(path){
      ...
      $.ajax(path+"/limit/10.json").success(function(data){
        ...
        $.each(data, function(key, value) {
          var mood = {"id":i,"val":value.mood.mood,"desc":value.mood.desc,"date":value.mood.report_time};
          if(i==0){
            //latest mood
            var moodStr = '<b>'+getMoodStr(mood.val)+'</b>';
            var status = name +' is '+moodStr+' : '+mood.desc;
        ...
        $.ajax(path).success(function(data){
            ...
            $.each(data, function(id, mood_json) {
                 ...
                 if(happy.length > 0){
                     ...
                     $("#happy-friends").append("<div class='no-data'>No Data Found</div>");
                     $("#happy-friends").css('background-color','white');
                 }
                 if(gloomy.length > 0){
                     $("body").data('gloomy-friends',gloomy);
                 }else{
                     $("#gloomy-friends").append("<div class='no-data'>No Data Found</div>");
                     $("#gloomy-friends").css('background-color','white');
                 }
            });
    ...
    function renderCloud(){
       var word_list = [
          {text: "Jonathan", weight: 9},
          {text: "Hot", weight: 2},
          {text: "Meeting", weight: 9},
          {text: "Friends", weight: 8},
          {text: "Delicious", weight: 5},
          {text: "Love", weight: 3},
          {text: "Funny", weight: 11},
          {text: "Great", weight: 6}
      ];
